...
import { McpTestClient, TestCase } from './McpTestClient';

// 配置
const TEST_DATA_DIR = path.join(__dirname, '..', '..', 'src', 'test', 'testData', 'mcp');
...
suite('MCP Tools Test Suite', function () {
  ...
  test('searchSymbolInWorkspace', () => runToolTest(client, 'searchSymbolInWorkspace'));
 ...
 * 运行工具测试
 */
async function runToolTest(client: McpTestClient, toolName: string): Promise<void> {
 ...
 * 替换测试参数中的占位符
 */
function replaceProjectPath(args: Record<string, unknown>): Record<string, unknown> {
 ...
 * 文件操作测试（需要备份/恢复测试目录）
 */
async function runToolWithFileContext(client: McpTestClient, toolName: string): Promise<void> {
  // 备份测试目录
