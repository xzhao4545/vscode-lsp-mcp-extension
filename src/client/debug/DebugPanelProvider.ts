...
export class DebugLogItem extends vscode.TreeItem {
  constructor(
    public readonly entry: DebugLogEntry
  ) {
    ...
    this.iconPath = new vscode.ThemeIcon(
      ...
      entry.success ? new vscode.ThemeColor('testing.iconPassed') : new vscode.ThemeColor('testing.iconFailed')
    );
  ...
  private buildTooltip(): vscode.MarkdownString {
    ...
    md.appendMarkdown(`**参数**: \`${JSON.stringify(this.entry.args).slice(0, 100)}...\``);
    return md;
  }
}

export class DebugPanelProvider implements vscode.TreeDataProvider<DebugLogItem> {
  private readonly _onDidChangeTreeData = new vscode.EventEmitter<DebugLogItem | undefined>();
