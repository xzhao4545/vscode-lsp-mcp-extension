...
export class GetSymbolStructTool extends BaseTool {
  ...
  private findSymbolAtPosition(
    ...
    for (const symbol of symbols) {
      if (symbol.range.start.line <= position.line && symbol.range.end.line >= position.line) {
        // 先在子符号中查找更精确的匹配
   ...
   */
  private autoAdjustDepth(symbol: SymbolInfo, maxPossibleDepth: number, maxLines: number): GetSymbolStructResult {
   ...
   */
  private applyDepthWithCollapse(symbol: SymbolInfo, targetDepth: number): GetSymbolStructResult {
    const processedSymbol = this.processSymbolWithDepth(symbol, 1, targetDepth);
    const hasCollapsed = Boolean(processedSymbol?.collapsed) ||
      Boolean(processedSymbol?.children && this.checkHasCollapsed(processedSymbol.children));
   ...
   * 固定深度模式
   */
  private applyFixedDepth(symbol: SymbolInfo, targetDepth: number): GetSymbolStructResult {
    const processedSymbol = this.processSymbolWithDepth(symbol, 1, targetDepth);
    const hasCollapsed = Boolean(processedSymbol?.collapsed) ||
      Boolean(processedSymbol?.children && this.checkHasCollapsed(processedSymbol.children));
    return { symbol: processedSymbol, hasCollapsed };
  ...
  format(result: GetSymbolStructResult): string {
    ...
    if (result.hasCollapsed) {
      sb.appendLine();
      sb.appendLine('*Some symbols are collapsed. Use `getSymbolStruct` with deeper `maxDepth` or line:character of collapsed symbol to expand.*');
    }
