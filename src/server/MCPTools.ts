...
export const TOOL_SCHEMAS = {
	listOpenProjects: {
		...
		inputSchema: z.object({
			projectPath: z
				...
				.optional()
				.describe(
					"Optional, used to query all project paths opened in the workspace corresponding to the specified project path.",
				),
		}),
	},
	...
	goToDefinition: {
		description:
			"Navigate to symbol definition. Prefer this over grep for finding where a variable/method/class is defined.",
		inputSchema: z.object({
			projectPath,
			filePath,
			line,
	...
	hover: {
		description:
			"Get hover information (name and doc comment) for a symbol. Use this to understand what a symbol is.",
		inputSchema: z.object({
			projectPath,
			filePath,
			line,
	...
	getFileStruct: {
		description:
			"Retrieve the file structure in a tree format. If you want to quickly understand the file structure, you should use this method instead of reading the entire file directly.\n" +
		...
		inputSchema: z.object({
			...
			maxDepth: z
				...
				.optional()
				.describe(
					"Depth limit. Negative (default -1) = auto mode (adjust depth to fit maxStructLines). Positive = fixed depth, ignores maxStructLines.",
				),
		}),
	},
	...
	getSymbolStruct: {
		...
		inputSchema: z.object({
			...
			maxDepth: z
				...
				.optional()
				.describe(
					"Depth limit. Negative (default -1) = auto mode. Positive = fixed depth.",
				),
		}),
	},
	...
	renameSymbol: {
		description:
			"Prepare rename edits for a symbol. Returns all locations to change without applying. Use this for safe refactoring across the project.",
		inputSchema: z.object({
			projectPath,
			filePath,
			line,
	...
	getDiagnostics: {
		description:
			"Get code diagnostics (errors, warnings) for a file. You should call this method after modifying the file to check for syntax errors.",
		inputSchema: z.object({
			projectPath,
			filePath,
			severity: z
				...
				.optional()
				.describe(
					"Min severity filter: Error|Warning|Information|Hint (default: Warning)",
				),
			page,
		}),
	},
	...
	getDefinitionText: {
		description:
			"Retrieve the complete definition text of the specified element (whether at its declaration or reference point), " +
	...
	syncFiles: {
		description:
			"Force IDE to sync virtual file system with external changes. Call after files are created/modified/deleted outside IDE.",
		inputSchema: z.object({
			projectPath,
			paths: z
				.array(z.string())
				.optional()
				.describe("File/directory paths to sync (relative to project)."),
		}),
	},
	...
	searchFiles: {
		...
		inputSchema: z.object({
			...
			directory: z
				...
				.optional()
				.describe(
					"Directory to search (relative to project, empty for project root)",
				),
			recursive: z
				.boolean()
				.optional()
	...
	getScopeParent: {
		...
		inputSchema: z.object({
			...
			line: z
				.number()
				.describe(
					"Line number (1-based), can be method/class definition or inside method/class body",
				),
		}),
	},
} as const;
