...
export class TaskManager {
  ...
  async dispatch(
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`[TaskManager] Dispatched ${tool} to ${client.windowId} (${requestId})`);
    });
  }
   ...
   * 处理错误
   */
  handleError(requestId: string, error: { message: string; code?: string }): void {
    const task = this.pending.get(requestId);
    if (task) {
      clearTimeout(task.timeout);
      this.pending.delete(requestId);
      task.reject(new Error(error.message || 'Task failed'));
      console.log(`[TaskManager] Error received for ${requestId}: ${error.message}`);
    }
  }
