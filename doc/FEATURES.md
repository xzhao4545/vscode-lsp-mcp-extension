...
**返回：**
- 未传 `projectPath`：返回 `workspaces`
- 传入 `projectPath`：仅返回匹配的 `targetWorkspace`
...
**注意：** 工具会自动预热并重试工作区符号索引，通常无需手动重试。

**输入参数：**
- `projectPath`: 项目目录绝对路径
- `query`: 搜索关键词（大小写不敏感子串匹配）
- `symbolType`: 符号类型过滤，可选值：`'class'`、`'method'`、`'field'`、`'all'`（默认 `'all'`）
  - `class`：类型声明集合，包含 `Class`、`Interface`、`Enum`、`Struct`
  - `method`：方法、函数、构造函数
  - `field`：字段、属性、变量、常量、枚举成员
  - `all`：不过滤
...
**注意：** 依赖 IDE Call Hierarchy；若语言服务未为该符号提供调用层级信息，结果可能为空。
...
1. **文件未打开**：先打开文档；如果后台打开仍未产生诊断，会临时激活该文件，等待语言服务器生成诊断
2. **文件已打开**：短暂等待确保诊断已生成
