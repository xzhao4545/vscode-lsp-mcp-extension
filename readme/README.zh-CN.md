...
- 17 个 MCP Tools，覆盖跳转、搜索、诊断与安全重构等常用代码智能操作
...
- `moveFile` 与 `deleteFile` 默认是安全模式，除非在设置中显式允许，否则需要确认。
...
- `getDiagnostics` 在必要时会等待语言服务器生成诊断结果，等待时长由 `ide-lsp-mcp.diagnosticsTimeout` 控制。
